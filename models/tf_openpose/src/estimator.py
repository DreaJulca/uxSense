...
class TfPoseEstimator:
    ...
    @staticmethod
    def draw_joints(npimg, humans, imgcopy=False):
        
        if imgcopy:
            npimg = np.copy(npimg)

        image_h, image_w = npimg.shape[:2]
        imgRaw = Image.new('RGBA', (image_w, image_h), (0, 0, 0, 255))
        img = np.array(imgRaw)
        ...
        for human in humans:
            ...
            for pair_order, pair in enumerate(common.CocoPairsRender):
                ...
                img = cv2.line(img, centers[pair[0]], centers[pair[1]], common.CocoColors[pair_order], 3)
    ...
    def pose_coordinates(npimg, humans, imgcopy=False):
        if imgcopy:
            npimg = np.copy(npimg)
        image_h, image_w = npimg.shape[:2]
        centers = {}
